...
def askfilesfirst_d(func):
    def tmp(*args, **kwargs):
        if kwargs.get("add_files") == True:
            kwargs["add_files"] = QFileDialog.getOpenFileNames(
                None,
                f"Choose {args[1].capitalize()} File(s)",
            )[0]
...
def working_d(func):
    def wrapper(self, *args, **kwargs):
        ...
        queue_.put(1)
        if not queue_.empty():
            mw.signalclass.setindicator.emit(
...
locks = {
    key: threading.RLock()
    for key in ("exp_df", "cat_df", "lin_df", "windows", "currThread", "axs", "meas")
}
...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.signalclass.notification.connect(
            lambda text: self.notificationsbox.add_message(text)
        )
        ...
        for key, label in {
            ...
            widget = QQ(
                QToolButton,
                text=label,
                change=lambda x, key=key: ws.send({"action": "state", "state": key}),
            )
    ...
    def dropEvent(self, event):
        ...
        for file in files:
            if not os.path.isfile(file):
                mw.notification(
                    f"<span style='color:#ff0000;'>ERROR</span>: The file {file} could not be found."
                )
                continue
    ...
    def dropEvent_core(self, files_by_class):
        ...
        for type in ["exp", "cat", "lin"]:
            ...
            if files:
                threads.append(
                    mw.load_file(type, keep_old=True, add_files=files, skip_update=True)
                )
    ...
    def websocketaction(self, message):
        ...
        if action == "measurement":
            ...
            error = message["error"]
            self.notification(
                f"<span style='color:#eda711;'>EXPERIMENT WARNING</span>: {error}"
            )

        elif action == "uerror":
            error = message["error"]
            self.notification(
                f"<span style='color:#ff0000;'>EXPERIMENT ERROR</span>: {error}"
            )
            ...
            self.update_state("disconnected")
            self.notification(
                f"<span style='color:#ff0000;'>CONNECTION ERROR</span>: {error}"
            )
            ...
            self.notification(
                f"<span style='color:#ff0000;'>ERROR</span>: Received a message with the unknown action '{action}' {message=}."
            )
    ...
    def get_measurement_data(self):
        ...
        measurement = {
            ...
            if key.split("_")[0] in key_prefixes
        }
        return measurement
    ...
    def savemeasurement(self, fname=None):
        if fname is None:
            fname = QFileDialog.getSaveFileName(
                None,
                "Choose file to save measurement to",
                "",
                "Measurement File (*.meas);;All Files (*)",
            )[0]
        if fname:
    ...
    def get_visible_data(
        self, type, xrange=None, binning=False, force_all=False, scale=True
    ):
        if type == "exp":
            with locks["exp_df"]:
        ...
        if force_all != True:
            visible_files = {
                file for file in fd.keys() if not fd[file].get("hidden", False)
            }
        ...
        if scale and type in ["exp", "cat"]:
            scalingfactordict = {
                file: self.config[f"files_{type}"][file].get("scale", 1)
                for file in fd.keys()
            }
    ...
    def createmenu(self):
        menus = {
            label: self.menuBar().addMenu(f"&{label}")
            for label in ("Files", "View", "Plot", "Actions", "Info")
        }
        ...
        self.pause_after_abort_action = QQ(
            QAction,
            parent=self,
            checkable=True,
            text="&Pause after aborting",
            tooltip="If experiment should pause after aborting measurement",
            change=lambda x: ws.send({"action": "pause_after_abort"}),
        )
        ...
        actions_to_menus = {
            "Files": (
                QQ(
                    ...
                    parent=self,
                    text="&Load Spectrum",
                    change=lambda x: self.load_file("exp", add_files=True),
                    tooltip="Replace Exp file(s)",
                ),
                QQ(
                    QAction,
                    parent=self,
                    text="&Add Spectrum",
                    change=lambda x: self.load_file(
                        "exp", add_files=True, keep_old=True
                    ),
                    tooltip="Add Exp file(s)",
                ),
                QQ(
                    QAction,
                    parent=self,
                    text="&Load Cat File",
                    change=lambda x: self.load_file("cat", add_files=True),
                    tooltip="Replace Cat file(s)",
                ),
                QQ(
                    QAction,
                    parent=self,
                    text="&Add Cat File",
                    change=lambda x: self.load_file(
                        "cat", add_files=True, keep_old=True
                    ),
                    tooltip="Add Cat file(s)",
                ),
                QQ(
                    QAction,
                    parent=self,
                    text="&Load Lin File",
                    change=lambda x: self.load_file("lin", add_files=True),
                    tooltip="Replace Lin file(s)",
                ),
                QQ(
                    QAction,
                    parent=self,
                    text="&Add Lin File",
                    change=lambda x: self.load_file(
                        "lin", add_files=True, keep_old=True
                    ),
                    tooltip="Add Lin file(s)",
                ),
                None,
                QQ(
                    QAction,
                ...
                QQ(
                    ...
                    text="&Edit Files",
                    shortcut="Shift+7",
                    tooltip="See current files and their options",
                    change=lambda x: self.filewindow.show(),
                ),
                None,
                QQ(
                    QAction,
            ...
            "View": (
                QQ(
                    ...
                    text="&Config Window",
                    shortcut="Shift+7",
                    tooltip="Show the config window",
                    change=lambda x: self.configwindow.show()
                    and self.configwindow.activateWindow(),
                ),
                QQ(
                    QAction,
                    "layout_mpltoolbar",
                    ...
                    text="&MPL Toolbar",
                    shortcut="Shift+1",
                    tooltip="Show or hide toolbar to edit or save the plot canvas",
                    checkable=True,
                ),
                toggleaction_queue,
                toggleaction_log,
                QQ(
                    ...
                    text="&Hover Window",
                    shortcut="Shift+6",
                    tooltip="Show the hover window",
                    change=lambda x: self.hoverwindow.show()
                    and self.hoverwindow.activateWindow(),
                ),
                None,
                QQ(
                    QAction,
                    ...
                    parent=self,
                    text="&Force Show Log",
                    tooltip="Make log window visible if a new message is shown",
                    checkable=True,
                ),
            ),
            "Plot": (
                QQ(
                    ...
                    tooltip="Set Center",
                    shortcut="Ctrl+G",
                    change=lambda x: self.plotwidget.position_dialog(),
                ),
                QQ(
                    QAction,
                    parent=self,
                    text="&Set Width",
                    tooltip="Set specific width",
                    shortcut="Ctrl+W",
                    change=lambda x: self.plotwidget.width_dialog(),
                ),
                None,
                QQ(
                    QAction,
                    ...
                    parent=self,
                    text="&Automatic Draw",
                    tooltip="Update canvas automatically when plot is updated, might be switched off if program is unresponsive",
                    checkable=True,
                ),
                QQ(
                    QAction,
                    parent=self,
                    text="&Manual Draw",
                    tooltip="Draw canvas manually",
                    change=lambda x: self.plotwidget.manual_draw(),
            ...
            "Actions": (
                QQ(
                    QAction,
                    parent=self,
                    text="&Save Measurement Values",
                    shortcut="Ctrl+S",
                    tooltip="Save current measurement values to file",
                    change=lambda x: self.savemeasurement(),
                ),
                QQ(
                    QAction,
                    parent=self,
                    text="&Load Measurement Values",
                    shortcut="Ctrl+O",
                    tooltip="Open measurement and set values accordingly",
                    change=lambda x: self.loadmeasurement(),
                ),
                None,
                QQ(
                    QAction,
                    ...
                    change=lambda x: self.queuewindow.savequeue(),
                ),
                QQ(
                    QAction,
                    parent=self,
                    text="&Load Queue",
                    tooltip="Load queue from file",
                    change=lambda x: self.queuewindow.loadqueue(),
                ),
                None,
                QQ(
                    QAction,
                    ...
                    change=lambda x: self.saveoptions(),
                ),
                QQ(
                    QAction,
                    parent=self,
                    text="&Clear Log",
                    tooltip="Clear the log messages in the logwindow",
                    change=lambda x: self.logwindow.clearlog(),
                ),
                QQ(
                    QAction,
                    parent=self,
                    text="&Open Measurements Folder",
                    tooltip="Open folder where measurements are stored",
                    change=lambda x: ws.send({"action": "measurements_folder"}),
                ),
                None,
                ...
                QQ(
                    QAction,
                    parent=self,
                    text="&Pop current measurement",
                    tooltip="Put current measurement into queue",
                    change=lambda x: ws.send({"action": "pop_measurement"}),
                ),
                None,
                QQ(
                    QAction,
                    ...
                    text="&Next Frequency",
                    shortcut="Ctrl+N",
                    tooltip="Go to next frequency while pausing",
                    change=lambda x: ws.send({"action": "next_frequency"}),
                ),
                QQ(
                    QAction,
                    parent=self,
                    text="&Reconnect Experiment",
                    tooltip="Reconnect the websocket to the experiment",
                    change=lambda x: ws.start(),
                ),
            ),
            "Info": (
                QQ(
                    ...
                    change=lambda x: send_mail_to_author(),
                ),
                QQ(
                    QAction,
                    parent=self,
                    text="&Credits and License",
                    tooltip="See the Credits and License",
                    change=lambda x: self.creditswindow.show(),
                ),
            ),
        }
    ...
    def load_file(
        ...
        if self.config["flag_loadfilesthreaded"]:
            ...
            for fname in fnames:
                t = threading.Thread(
                    target=self.load_file_core,
                    args=(fname, type, config_updates, results, errors, do_QNs),
                )
                t.start()
                threads.append(t)
            ...
            for fname in fnames:
                ...
                try:
                    self.load_file_core(
                        fname, type, config_updates, results, errors, do_QNs
                    )
                except Exception as E:
                    pass
        ...
        with lock:
            ...
            results.put(df)
            if type == "exp":
                self.exp_df = pd.concat(
                    list(results.queue), ignore_index=True
                ).sort_values("x", kind="merge")
            elif type == "cat":
                self.cat_df = pd.concat(
                    list(results.queue), ignore_index=True
                ).sort_values("x", kind="merge")
            elif type == "lin":
                self.lin_df = pd.concat(
                    list(results.queue), ignore_index=True
                ).sort_values("x", kind="merge")
        ...
        if len(fnames):
            error_text = (
                f"<span style='color:#ff0000;'>ERROR</span>: Reading {type.capitalize()} files not successful. "
                if len(errors) != 0
                else ""
            )
            self.notification(
                f"{error_text}Read {str(len(fnames)-len(errors))+'/' if len(errors) != 0 else ''}{len(fnames)} {type.capitalize()} files successfully."
            )
    ...
    def load_file_core(self, fname, type, config_updates, results, errors, do_QNs):
        try:
            if not os.path.isfile(fname):
                errors.put(fname)
                self.notification(
                    f"<span style='color:#ff0000;'>ERROR</span>: The file {fname} could not be found. Please check the file."
                )
            ...
            if type == "exp":
                args = (
                    chr(self.config["flag_separator"]),
                    self.config["flag_xcolumn"],
                    self.config["flag_ycolumn"],
                    False,
                )
            ...
            self.notification(
                f"<span style='color:#ff0000;'>ERROR</span>: There occurred an error when loading the {type.capitalize()} File {fname}. Please check the file."
            )
...
class PlotWidget(QGroupBox):
    def __init__(self, parent):
        ...
        mw.config.register(
            "layout_mpltoolbar",
            lambda: self.mpltoolbar.setVisible(mw.config["layout_mpltoolbar"]),
        )
        ...
        for label, func in buttonsdict.items():
            button = QQ(
                ...
                text=label,
                change=func,
                visible=mw.config["flag_showmainplotcontrols"],
            )
            toplayout.addWidget(button)
            mw.config.register(
                "flag_showmainplotcontrols",
                lambda button=button: button.setVisible(
                    mw.config["flag_showmainplotcontrols"]
                ),
            )
        ...
        self.meas_coll = matplotlib.collections.LineCollection(
            np.zeros(shape=(0, 2, 2)), colors=mw.config["color_meas"]
        )
        self.meas_coll_scatter = self.ax.scatter(
            [], [], color=mw.config["color_meas"], marker="x", zorder=100
        )
        ...
        self.plots = {
            ...
            "cat": None,
            "lin": self.ax.scatter(
                [], [], color=mw.config["color_lin"], marker="*", zorder=100
            ),
        }
        ...
        mw.config.register(
            "flag_updateplot",
            lambda: (
                self.timer.start(max(100, mw.config["flag_updateplot"]))
                if mw.config["flag_updateplot"] > 0
    ...
    def position_dialog(self):
        ...
        try:
            ...
            mw.notification(
                "<span style='color:#eda711;'>WARNING</span>: The entered value could not be interpreted as a number."
            )
            return
    ...
    def get_meas_data(self, filtered=True):
        ...
        if filtered:
            ...
            mw.signalclass.progressbar.emit(
                int(index_ / self.meas_array.shape[0] * 100)
            )
    ...
    def set_data_core(self, manual_draw=False):
        ...
        try:
            ...
            dataframes = {
                key: mw.get_visible_data(
                    key, xrange=(xmin, xmax), binning=True, scale=True
                )
            ...
            if autoscale:
                ...
                ys = (
                    ys
                    * mw.config["plot_expcat_factor"]
                    * 10 ** mw.config["plot_expcat_exponent"]
                )
            segs = np.array(((xs, xs), (ys * 0, ys))).T
    ...
    def set_meas_data(self, standalone=True, manual_draw=False):
        ...
        if mw.config["plot_showmagnitude"]:
            xs, ys = meas_data[:, 0], np.sqrt(
                (meas_data[:, 2]) ** 2 + (meas_data[:, 3]) ** 2
            )
        else:
            ...
            for type, df in dataframes.items():
                if len(df):
                    ...
                    for i, row in df.iterrows():
                        ...
                        tmp.append(
                            f"{', '.join([str(qn) for qn in qnus if qn != pyckett.SENTINEL])} ← {', '.join([str(qn) for qn in qnls if qn != pyckett.SENTINEL])}"
                        )
...
class Config(dict):
    ...
    def callback(self, args):
        ...
        if widget:
            callbacks_widget = self.callbacks.query(
                f"key == @key and widget != @widget"
            )
        else:
...
class QSweep(QWidget):
    ...
    def setState(self, state):
        ...
        if state["mode"] == "fixed":
            ...
            self.widgets["Direction"].setCurrentText(
                state.get("direction", "forthback")
            )
...
class QueueWindow(EQDockWidget):
    def __init__(self, parent):
        ...
        self.buttons = {
            key: QQ(QToolButton, text=key, change=lambda x, key=key: self.command(key))
            for key in keys
        }
    ...
    def command(self, key):
        ...
        if key == "Add last":
            ...
            reply = QMessageBox.question(
                self,
                "Delete all",
                "Are you sure you want to delete all pending measurements?",
            ...
            for i, row in enumerate(list_):
                ...
                for header, value in update_dict.items():
                    if header in tmp_measurement.keys():
                        ...
                        raise CustomError(
                            f"Did not understand the header {header} in you list."
                        )
                ...
                if i % mw.config["flag_maxmeasurementspermessage"] == 0:
                    ws.send(
                        {"action": "add_measurements", "measurements": measurements}
                    )
                    measurements = []
            ...
            if dialog.result() == 1:
                ...
                add_reproducibility_measurements = (
                    result["reproducibility"]["every"] > 0
                )
                ...
                if add_reproducibility_measurements:
                    ...
                    for source in ("probe", "pump"):
                        if not result["reproducibility"].get(source):
                            continue
                        center, span = result["reproducibility"][source].get(
                            "center"
                        ), result["reproducibility"][source].get("span")
                        if center and span:
                            reproducibility_measurement[f"{source}_frequency"][
                                "center"
                            ] = center
                            reproducibility_measurement[f"{source}_frequency"][
                                "span"
                            ...
                            for key in ("start", "stop"):
                                if (
                                    key
                                    in reproducibility_measurement[
                                        f"{source}_frequency"
                                    ]
                                ):
                                    reproducibility_measurement[f"{source}_frequency"][
                    ...
                    for i_probe in range(result["probe"]["measurements"]):
                        ...
                        for source in ("probe", "pump"):
                            ...
                            if "center" in new_values and "span" in new_values:
                                ...
                                values["center"] = (
                                    new_values["center"] + i * new_values["span"]
                                )
                        ...
                        if add_reproducibility_measurements and (
                            total_index % reproducibility_measurement_every == 0
                        ):
                            measurements.append(reproducibility_measurement)

                        if (
                            len(measurements)
                            >= mw.config["flag_maxmeasurementspermessage"]
                        ):
                            ws.send(
                                {
    ...
    def savequeue(self):
        fname = QFileDialog.getSaveFileName(
            None,
            "Choose file to save queue to",
            "",
            "Queue File (*.queue);;All Files (*)",
        )[0]
        if not fname:
            return
    ...
    def loadqueue(self):
        fname = QFileDialog.getOpenFileName(
            None, "Choose queue to load", "", "Queue File (*.queue);;All Files (*)"
        )[0]
        if not fname:
            return
...
class BatchDialog(QDialog):
    def __init__(self):
        ...
        for source in ("probe", "pump"):
            ...
            if tmp_dict["mode"] != "sweep":
                layout.addWidget(
                    QQ(
                        QLabel,
                        text=f"Please choose sweep as mode in the {Source} panel to activate this section.",
                        wordwrap=True,
                    ),
                    current_row,
                    0,
                ...
                if "span" in tmp_dict and "center" in tmp_dict:
                    ...
                    self.widgets[source]["center"] = tmp = QQ(
                        QDoubleSpinBox,
                        range=(0, None),
                        value=center,
                        change=self.update,
                    ...
                    layout.addWidget(
                        QQ(QLabel, text="Center first Measurement: "), current_row, 0
                    )
                    layout.addWidget(tmp, current_row, 1)
                    current_row += 1

                    self.widgets[source]["span"] = tmp = QQ(
                        QDoubleSpinBox, range=(0, None), value=span, change=self.update
                    )
                    layout.addWidget(
                        QQ(QLabel, text="Span first Measurement: "), current_row, 0
                    )
                    layout.addWidget(tmp, current_row, 1)
                    current_row += 1
                    ...
                    self.widgets[source]["start"] = tmp = QQ(
                        QDoubleSpinBox, range=(0, None), value=start, change=self.update
                    )
                    layout.addWidget(
                        QQ(QLabel, text="Start first Measurement: "), current_row, 0
                    )
                    layout.addWidget(tmp, current_row, 1)
                    current_row += 1

                    self.widgets[source]["stop"] = tmp = QQ(
                        QDoubleSpinBox, range=(0, None), value=stop, change=self.update
                    )
                    layout.addWidget(
                        QQ(QLabel, text="Stop first Measurement: "), current_row, 0
                    )
                    layout.addWidget(tmp, current_row, 1)
                    current_row += 1

                self.widgets[source]["measurements"] = tmp = QQ(
                    QSpinBox, range=(1, None), change=self.update
                )
                layout.addWidget(
                    QQ(QLabel, text="Number of Measurements: "), current_row, 0
                )
                layout.addWidget(tmp, current_row, 1)
                current_row += 1

                self.widgets[source]["starttotal"] = tmp = QQ(
                    QDoubleSpinBox, range=(0, None), enabled=False
                )
                layout.addWidget(
                    QQ(QLabel, text="Start Frequency All: "), current_row, 0
                )
                layout.addWidget(tmp, current_row, 1)
                current_row += 1

                self.widgets[source]["stoptotal"] = tmp = QQ(
                    QDoubleSpinBox, range=(0, None), enabled=False
                )
                layout.addWidget(
                    QQ(QLabel, text="Stop Frequency All: "), current_row, 0
                )
                layout.addWidget(tmp, current_row, 1)
                current_row += 1
        ...
        self.widgets["reproducibility"]["every"] = tmp = QQ(
            QSpinBox, range=(0, None), value=0, change=self.update
        )
        ...
        for source in ("probe", "pump"):
            ...
            self.widgets["reproducibility"][source]["center_label"] = tmp = QQ(
                QLabel, text=f"Center {Source}: "
            )
            layout.addWidget(tmp, current_row, 0)
            self.widgets["reproducibility"][source]["center"] = tmp = QQ(
                QDoubleSpinBox, range=(0, None), value=center
            )
            layout.addWidget(tmp, current_row, 1)
            current_row += 1

            self.widgets["reproducibility"][source]["span_label"] = tmp = QQ(
                QLabel, text=f"Span {Source}: "
            )
            layout.addWidget(tmp, current_row, 0)
            self.widgets["reproducibility"][source]["span"] = tmp = QQ(
                QDoubleSpinBox, range=(0, None), value=span
            )
            layout.addWidget(tmp, current_row, 1)
            current_row += 1
        ...
        for source in ("probe", "pump"):
            ...
            if widgets:
                ...
                if "center" in widgets and "span" in widgets:
                    ...
                    start_total, stop_total = center - span * 0.5, center + span * (
                        measurements - 0.5
                    )
                else:
    ...
    def save(self):
        ...
        results["reproducibility"]["every"] = self.widgets["reproducibility"][
            "every"
        ].value()
        for source in ("probe", "pump"):
    ...
    def __init__(self, parent):
        ...
        self.widgets = {
            "Probe Device": QQ(
                QComboBox,
                "static_probedevice",
                options=devices.deviceclasses["probe"].keys(),
            ),
            "Probe Multiplication": QQ(
                QSpinBox, "static_probemultiplication", range=(1, None)
            ),
            "Probe Address": QQ(QLineEdit, "static_probeaddress"),
            "LockIn Device": QQ(
                QComboBox,
                "static_lockindevice",
                options=devices.deviceclasses["lockin"].keys(),
                change=mw.signalclass.lockintypechanged.emit,
            ),
            "LockIn Address": QQ(QLineEdit, "static_lockinaddress"),
            "Pump Device": QQ(
                QComboBox,
                "static_pumpdevice",
                options=devices.deviceclasses["pump"].keys(),
            ),
            "Pump Multiplication": QQ(
                QSpinBox, "static_pumpmultiplication", range=(1, None)
            ),
...
class LockInWindow(MeasWidget):
    def __init__(self, parent):
        ...
        self.tc_widget = QQ(
            QComboBox,
            "lockin_timeconstant",
            options=devices.SignalRecovery7265.TC_OPTIONS,
        )
        self.sen_widget = QQ(
            QComboBox,
            "lockin_sensitivity",
            options=devices.SignalRecovery7265.SEN_OPTIONS,
        )
        self.acgain_widget = QQ(
            QComboBox,
            "lockin_acgain",
            options=devices.SignalRecovery7265.ACGAIN_OPTIONS,
        )
    ...
    def update_lockin_options(self):
        ...
        for key, class_ in devices.deviceclasses["lockin"].items():
            ...
            raise ValueError(
                "Did not find your currently used lockin in the devices module."
            )
...
class ConfigWindow(EQWidget):
    def __init__(self, id, parent=None):
        ...
        tmp_layout.addWidget(
            QQ(QPushButton, text="Save as default", change=lambda: mw.saveoptions())
        )
...
class FileWindow(EQWidget):
    ...
    def create_layout(self, type, initial=False):
        if initial:
            ...
            buttonsbox.addWidget(
                QQ(
                    QToolButton,
                    text="Load",
                    change=lambda x, type=type: mw.load_file(
                        type, add_files=True, keep_old=False
                    ),
                )
            )
            buttonsbox.addWidget(
                QQ(
                    ...
                    text="Add",
                    change=lambda x, type=type: mw.load_file(
                        type, add_files=True, keep_old=True
                    ),
                )
            )
            buttonsbox.addWidget(
            ...
            buttonsbox.addWidget(
                QQ(
                    QToolButton,
                    text="Reset All",
                    change=lambda x, type=type: self.reset_all(type),
                )
            )
            buttonsbox.addWidget(
                QQ(
                    QToolButton,
                    text="Delete All",
                    change=lambda x, type=type: self.delete_file(type),
                )
            )
            ...
            rowdict = {
                ...
                "colorinput": QQ(
                    ...
                    text=color,
                    maxWidth=200,
                    change=lambda x, file=file, type=type: self.change_color(
                        type, file, inp=True
                    ),
                ),
                "colorpicker": QQ(
                    ...
                    stylesheet=f"background-color: {rgbt_to_trgb(color)}",
                ),
            }
    ...
    def add_row(self, layout, type, file, actions, row_id):
        ...
        rowdict = {
            ...
            "scale": QQ(
                ...
                value=scale,
                range=(None, None),
                change=lambda x, file=file, type=type: self.scale_file(type, file, x),
            ),
            "colorinput": QQ(
                QLineEdit,
                text=color,
            ...
            "colorpicker": QQ(
                QToolButton,
                text="CP",
                change=lambda x, file=file, type=type: self.change_color(type, file),
                stylesheet=f"background-color: {rgbt_to_trgb(color)}",
            ),
            "hide": QQ(
                QToolButton,
                text="Show" if hidden else "Hide",
                change=lambda x, type=type, file=file: self.hide_file(type, file),
            ),
            "delete": QQ(
                QToolButton,
                text="×",
                change=lambda x, type=type, file=file: self.delete_file(type, file),
                tooltip="Delete file",
            ),
            "reread": QQ(
                QToolButton,
                text="⟲",
                change=lambda x, type=type, file=file: mw.load_file(
                    type, add_files=[file], keep_old=True, do_QNs=False
                ),
                tooltip="Reread File",
            ),
        }
    ...
    def change_color(self, type, file, inp=False):
        ...
        if inp:
            ...
            color = QColorDialog.getColor(
                initial=QColor(rgbt_to_trgb(color_input)),
        ...
        self.widgets[type][file]["colorpicker"].setStyleSheet(
            f"background-color: {rgbt_to_trgb(color)}"
        )
...
class ConsoleDialog(QDialog):
    def __init__(self):
        ...
        self.resize(
            mw.config["commandlinedialog_width"], mw.config["commandlinedialog_height"]
        )
        ...
        buttons_layout.addWidget(
            QQ(
                QPushButton,
                text="Run",
                change=lambda x: self.predone(1),
        ...
        buttons_layout.addWidget(
            QQ(
                QPushButton,
                text="Cancel",
                change=lambda x: self.predone(0),
                shortcut="Esc",
            )
        )
        buttons_layout.addStretch()
    ...
    def run(showdialog=True):
        ...
        title, command = mw.config["commandlinedialog_commands"][
            mw.config["commandlinedialog_current"]
        ]
        ...
        try:
            ...
            message.append(
                f"<span style='color:#eda711;'>WARNING</span>: Executing the code raised an error: {str(E)}"
            )
            raise
        finally:
...
def QQ(widgetclass, config_key=None, config_object=None, **kwargs):
    ...
    if config_key:
        ...
        changer(
            lambda x=None, key=config_key: config_object.__setitem__(
                key, getter(), widget
            )
        )
        config_object.register_widget(
            config_key, widget, lambda: setter(config_object[config_key])
        )
    if "change" in kwargs:
...
def exp_to_df(fname, sep="\t", xcolumn=0, ycolumn=1, sort=True):
    data = pd.read_csv(
        fname, sep=sep, dtype=np.float64, header=None, engine="c", comment="#"
    )
...
def bin_data(dataframe, binwidth, range):
    ...
    dataframe = dataframe.loc[
        dataframe.sort_values("y")
        .drop_duplicates(("bin", "filename"), keep="last")
        .sort_values(["x"])
        .index
    ]
    return dataframe
...
config_specs = {
    ...
    "flag_extensions": [
        {"exp": [".csv"], "cat": [".cat"], "lin": [".lin"], "measurement": [".meas"]},
        dict,
    ],
