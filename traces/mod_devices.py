...
class SCPIAttribute:
    ...
    def validate(self, value):
        ...
        if self.novalue:
            if value:
                raise ValueError(
                    "Trying to provide a value for a command with no argument."
                )

        if self.options:
            if value not in self.options:
                raise ValueError(
                    f"The value '{value}' is no valid option for this SCPI attribute."
                )
            ...
            if min_ > value:
                raise ValueError(
                    f"The value '{value}' is smaller than the allowed minimum '{min_}'."
                )
            if max_ < value:
                raise ValueError(
                    f"The value '{value}' is larger than the allowed maximum '{max_}'."
                )
            ...
            startvalues.update(
                {
                    ...
                    "SOUR:FM1:STAT": "ON",
                    "SOUR:FM1:DEV": str(
                        dict_["lockin_fmdeviation"] / self.multiplication
                    )
                    + "kHz",
                ...
                startvalues.update(
                    {
                        ...
                        "SOUR:LFO": "ON",
                        "SOUR:FM1:STAT": "ON",
                        "SOUR:FM1:DEV": dict_["general_dmjump"]
                        / dict_["static_pumpmultiplication"]
                        / 2,
                        "SOUR:LFO1:SHAP": "SQU",
                ...
                startvalues.update(
                    {
                        ...
                        "SOUR:AM1:DEPT": 100,
                        "SOUR:LFO1:SHAP": "SQU",
                        "SOUR:LFO1:SHAP:PULS:PER": str(dict_["general_dmperiod"])
                        + "ms",
                        "SOUR:LFO1:SHAP:PULS:DCYC": 50,
                    }
                )
            ...
            startvalues.update(
                {
                    # Set FM modulation
                    "SOUR:FM1:STAT": "ON",
                    "SOUR:FM1": str(dict_["lockin_fmdeviation"] / self.multiplication)
                    + "kHz",
                    # Set FM modulation signal
                    "SOUR:LFO:STAT": "ON",
                    "SOUR:LFO:AMPL": "1VP",
...
class SignalRecovery7265(LockInAmplifier, SCPIDevice):
    ...
    def measure_intensity_dmdr_digital(self):
        ...
        for state in (0, 1):
            ...
            while (
                time.perf_counter() - counterstart
                < self.timeconstant + additional_delay_time
            ):
                continue
    ...
    def prepare_measurement(self, dict_, devicetype):
        ...
        for key, options in (
            ("lockin_timeconstant", self.TC_OPTIONS),
            ("lockin_acgain", self.ACGAIN_OPTIONS),
            ("lockin_sensitivity", self.SEN_OPTIONS),
        ):
            value = dict_[key]
        ...
        locked_freq = lambda self=self: float(
            self.connection.query("FRQ.?").split("\n")[0]
        )
        ...
        while not abs(locked_freq() - actual_freq) / actual_freq < 0.01:
            ...
            if time.perf_counter() - timeout_start > 5:
                raise CustomError(
                    "Timed out when waiting for Lock-In to lock to reference signal"
                )
...
class ZurichInstrumentsMFLI(LockInAmplifier):
    ...
    def prepare_measurement(self, dict_, devicetype):
        ...
        if dict_["general_mode"] == "classic":
            ...
            self.daq.setInt(
                "/dev4055/demods/1/adcselect", 8
            )  # Aux1 as input for demod1
    ...
    def get_signal(
        self, samplefrequency, duration, records=1, timeout=2, timeoffset=True
    ):
        ...
        while (c_record < records) or (progress < 1.0):
            ...
            if (time.perf_counter() - st) > timeout:
                raise ValueError(
                    f"Recording the desired time signal took longer than the timeout of {timeout} s."
                )
                break
        ...
        result = [ts] + [
            record[0]["wave"][0] for record in data["/dev4055/scopes/0/wave"]
        ]
        return result
